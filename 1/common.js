...
var mcrp = {};
(function(mcrp, doc) {
	mcrp.IsNullOrEmpty = function(str) {
...
　};  
    mcrp.formatMobile=function(mobile){
    	if(!mcrp.IsNullOrEmpty(mobile)&&mobile.isMobile()){
    		return mobile.substr(0,3)+'****'+mobile.substr(7,4);
    	}else{
    		return mobile;
    	}
    };
     mcrp.checkIdCard = function(value) {
		var idCard = value;
		if (idCard.length == 15) {
			return mcrp.isValidityBrithBy15IdCard(idCard);
		} else if (idCard.length == 18) {
			var arrIdCard = idCard.split("");
			if (mcrp.isValidityBrithBy18IdCard(idCard) && mcrp.isTrueValidateCodeBy18IdCard(arrIdCard)) {
				return true;
			} else {
				return false;
			}
	...
	mcrp.isTrueValidateCodeBy18IdCard = function(arrIdCard) {
		var sum = 0;
	...
	mcrp.isValidityBrithBy18IdCard = function(idCard18) {
	...
	mcrp.isValidityBrithBy15IdCard = function(idCard15) {
	...
	mcrp.formatDate = function(formatStr, fdate) {
		if(!mcrp.IsNullOrEmpty(fdate)){
			if(~fdate.indexOf('.')){
	...
	mcrp.rmoney = function(money) {
		return parseFloat(money).toFixed(2).toString().split('').reverse().join('').replace(/(\d{3})/g, '$1,').replace(/\,$/, '').split('').reverse().join('');
	};
	mcrp.wait = function(type,mode,tips) {
		// type:是否div模式, mode:是否为模态,tips:提示信息
		if(type){
			var div=document.createElement('div');	
			div.className=mode?"mcrp-backdrop":"mcrp-loadding";
	  		div.innerHTML=mode?'<div class="mcrp-loadding"><span class="mui-spinner mui-spinner-white" ></span><span class="mcrp-loadtips">'+tips+'</span></div>':'<span class="mui-spinner mui-spinner-white" ></span><span class="mcrp-loadtips">'+tips+'</span>';
	...
	mcrp.closewait = function(w,type) {
		if(type&&w){
	...
	mcrp.weChat=function(){
		//return 'http://192.168.1.26:8090/';
		return 'http://1testm.qhlead.com/';
	};
	//接口地址
	mcrp.interfaceUrl=function(){
		//return 'http://192.168.1.160:8090/';
		return 'http://1testmapi.qhlead.com/';
	};
	mcrp.request=function(targetUrl,postData,way,callback,isNet,murl){
		var SERVER_URL=mcrp.interfaceUrl()+targetUrl;
		if(murl){
	...
	mcrp.post=function(targetUrl,postData,callback,isNet){
		var SERVER_URL=mcrp.interfaceUrl()+targetUrl;
		mui.ajax(SERVER_URL, {
	...
	mcrp.open=function(pageUrl,pageId,para,styles,NHeader,title,hasProgress,hasButtons,text,callback){
	...
	mcrp.fragment=function(){
		return doc.createDocumentFragment();
	}; 
	mcrp.unique=function(n){
      n=n || 6;
	  var rnd='';
	  for(var i=0;i<n;i++)
	...
	//1秒内不可重复创建相同页面
	mcrp.webviewId=function(){
	  return mcrp.formatDate('ymdhis',new Date().toLocaleString().replace(/[年月]/g,'-').replace(/[日上下午]/g,''));
	};
	mcrp.createtips=function(icon,word){
    ...
    mcrp.rmovetips=function(elem){
  	 	if(elem){
  	 		doc.body.removeChild(elem);
  	 	}
  	 	return null;
  	};
  	mcrp.updatetips=function(elem,word){
  		elem.querySelector('.tipsWord').innerText=word;
  	};
    mcrp.InitBadge = function(badge) {
	 plus.runtime.setBadgeNumber(badge);
  };
	mcrp.constNum = function() {
		return mui.os.ios?0:260;
	};	
	 mcrp.queryString=function(url,name){
	...
	function plusReadyCom(){
		  ...
		  mui.ajaxSettings.beforeSend=function(xhr,setting){
		  	  xhr.setRequestHeader("Token",mcrp.getToken());
